...
function DiaryProducts() {
 const [cards, setCards] = useState(DiaryProduct);
 ...
 return (
  <>
  <div style={{background: 'linear-gradient(90deg,#d9e7ff 25%, #e3ffe7 60% )'}}>
  <Navbar sticky="top"  expand="xxl" className="bg-primary-tertiary nav" style={{background: 'linear-gradient(90deg, #e3ffe7 0%, #d9e7ff 100%)',height:'100px'}}>
  <Container fluid>
    <Navbar.Brand href="#"><div style={{display:'flex',marginLeft:'80px'}}><img
            ...
            /><h1 style={{marginLeft:'30px',marginTop:'30px'}}>DiaryProducts</h1></div></Navbar.Brand>
    ...
    <div className="Product" style={{ marginTop: '30px', marginBottom: '20px',marginLeft:'25px',marginRight:'25px' }}>
      <Row xs={1} md={5}>
        {cards.map((card, index) => (
          <Col key={card.id}>
            <Card style={{ boxShadow: '2px 2px 3px 3px #fdf9e3' ,background: 'linear-gradient(90deg, #e3ffe7 0%, #d9e7ff 100%)'}} className="g-5 mt-5">
              <img src={card.image} alt="Card" style={{ height: '160', width: '306' }} />
              <Card.Body>
                <Card.Title style={{ backgroundColor: 'lightgreen', textAlign: 'center' }}>
                 {card.name}
                </Card.Title>
                <Form>
                ...
                <div className="text-center">
                 <button type="submit" className="btn  mt-2" style={{backgroundColor:'#fdf9e3'}} >
                    Save
                 </button>
                </div>
              </Card.Body>
